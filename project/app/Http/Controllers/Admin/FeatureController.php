    ...
    public function datatables()
    {
         ...
         return DataTables::of($datas)
                            ->editColumn('photo', function(Feature $data) {
                                $photo = $data->photo ? url('assets/images/feature/'.$data->photo):url('assets/images/noimage.png');
                            ...
                            ->addColumn('action', function(Feature $data) {
                                return '<div class="action-list"><a href="javascript:;" data-href="'. route('admin-slider-edit',$data->id) . '"  class="btn btn-primary btn-sm mr-2 edit" data-toggle="modal" data-target="#modal1"> <i class="fas fa-edit mr-1"></i>'. __('Edit') .'</a><a href="javascript:;" data-href="' . route('admin-slider-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i></a></div>';
                            })
                            ->editColumn('serial', function(Feature $data) {
    ...
    public function store(Request $request)
    {
        ...
        if ($file = $request->file('photo')) 
         {      
            ...
            $file->move('assets/images/feature',$name);           
            $input['photo'] = $name;
        } 
    ...
    public function update(Request $request, $id)
    {
        ...
        $input = $request->all();
            ...
            {              
                ...
                if($data->photo != null)
                {
                    if (file_exists(public_path().'/assets/images/feature/'.$data->photo)) {
                        unlink(public_path().'/assets/images/feature/'.$data->photo);
                    }
                }            
            $input['photo'] = $name;
            } 
