...
class CarController extends Controller
{
    ...
    public function datatables()
    {
        ...
        ->addColumn('action', function(Car $data) {
            ...
            <a href="'. route('admin-car-edit',$data->id) . '"  class="btn btn-primary btn-sm mr-2"> <i class="fas fa-edit mr-1"></i>'. __('Edit') .'</a><a href="javascript:;" data-href="' . route('admin-car-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i></a>
            </div>';
        })
        ...
        ->editColumn('serial', function(Car $data) {
    ...
    public function create()
    {
        ...
        return view('admin.car.create',compact('car_attrs','users','countries'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.car.edit',compact('car_attrs','main','users','countries'));
    }
        ...
        if($request->seo_check == 'yes'){
            ...
            // logic------------------------
                    ...
                    if($request->faq_title){
                        foreach($request->faq_title as $findex => $fac_title){
                            if($fac_title && $request->faq_content[$findex]){
                                $ftitle[] = $fac_title;
                                $fcontent[] = $request->faq_content[$findex];
                            }
                            $input['faq_title'] = Helper::Ximplode(array_filter($ftitle));
                            $input['faq_content'] = Helper::Ximplode(array_filter($fcontent));
                        }
                    }
    ...
    public function destroy($id)
    {
        ...
        if(count($data->reviews) > 0){
            foreach($data->reviews->where('type','car') as $review){
                $review->delete();
            }
        }
