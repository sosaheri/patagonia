...
class SocialSettingController extends Controller
{
    ...
    public function facebookup($status)
    {
        ...
        $data->f_check = $status;
        $data = $data->update();
        $data = Socialsetting::findOrFail(1)->f_check;
        $mgs = __('Data Updated Successfully.');
        return response()->json(['mgs' => $mgs , 'data' => $data]);
    }
    ...
    public function googleup($status)
    {
        ...
        $data->g_check = $status;
        $data->update();
        $data = Socialsetting::findOrFail(1)->g_check;
        $mgs = __('Data Updated Successfully.');
        return response()->json(['mgs' => $mgs , 'data' => $data]);
    }
