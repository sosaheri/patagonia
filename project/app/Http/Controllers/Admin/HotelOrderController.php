...
class HotelOrderController extends Controller
{
   public function __construct()
   {
    ...
    {
        return view('admin.hotel.orders.index');
    }
    ...
    public function datatables($type)
    {
        ...
        if($type == 'all'){
         $datas = Order::orderBy('id','desc')->where('order_type','Hotel')->get();
        }else{
         $datas = Order::orderBy('id','desc')->where('order_status',$type)->where('order_type','Hotel')->get();
        }
        ...
        ->editColumn('order_status', function (Order $data) {
         ...
         return '<div class="btn-group">
                     ...
                     <div class="dropdown-menu">
                         <a class="dropdown-item StatusCheck" data-val="Pending" href="javascript:;" data-href="'.route('admin-hotel-order-status',[ $data->id, 'Pending']).'">'.__('Pending').'</a>
                         <a class="dropdown-item StatusCheck" data-val="Processing" href="javascript:;" data-href="'.route('admin-hotel-order-status',[ $data->id, 'Processing']).'">'.__('Processing').'</a>
                         <a class="dropdown-item complete_check" data-toggle="modal" data-target="#complete_check" data-val="Completed" href="javascript:;" data-href="'.route('admin-hotel-order-status',[$data->id, 'Completed']).'">'.__('Completed').'</a>
                         <a class="dropdown-item StatusCheck" data-val="Cancle" href="javascript:;" data-href="'.route('admin-hotel-order-status',[$data->id, 'Cancle']).'">'.__('Cancle').'</a>
                         </div>
                  </div>';
         })
       ...
       ->addColumn('action', function(Order $data) {
           ...
           <a href="'.route('admin.hotel.order.details',$data->id).'"  class="btn btn-primary btn-sm mr-2 order_details" data-toggle="modal" data-target="#viewDetails">
               '.__('Details').'
           </a>
    ...
    {
      $data = Order::findOrFail($id);
      return view('admin.hotel.orders.details',compact('data'));
    }
