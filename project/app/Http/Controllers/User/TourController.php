...
class TourController extends Controller
{
    ...
    {
        return view('user.tour.index');
    }

    public function datatables()
    {
        $datas = TourModel::orderBy('id','desc')->where('author_id',Helper::user_id())->where('author_type','user')->get();
        ...
        ->addColumn('action', function(TourModel $data) {
            ...
            <a href="'. route('user-tour-edit',$data->id) . '"  class="btn btn-primary btn-sm mr-2"> <i class="fas fa-edit mr-1"></i>'. __('Edit') .'</a><a href="javascript:;" data-href="' . route('user-tour-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i></a></div>';
        })
        ...
        ->rawColumns(['status','action','main_price','location'])
        ->toJson();
    }
    ...
    public function create()
    {
        ...
        return view('user.tour.create',compact('data'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($request->seo_check == 'yes'){
            ...
            if($request->include){
                $input['include'] = Helper::Ximplode(array_filter($request->include));
            }
            if($request->exclude){
                $input['exclude'] = Helper::Ximplode(array_filter($request->exclude));
            }
    ...
    public function edit($id)
    {
        ...
        return view('user.tour.edit',compact('data','main'));
    }
