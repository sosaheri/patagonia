...
class TourOrderController extends Controller
{
   ...
   public function __construct()
   {
    ...
    public function orders(Request $request)
    {
      ...
      return view('user.tour.orders.index');
    }
    ...
    public function datatables($type)
    {
      $user = Auth::user();
      if($type == 'all'){
         $datas = Order::orderBy('id','desc')->where('order_type','Tour')->where('author_type','user')->where('author_id',$user->id)->get();
        }else{
         $datas = Order::orderBy('id','desc')->where('order_status',$type)->where('author_type','user')->where('author_id',$user->id)->where('order_type','Tour')->get();
        }
        ...
        ->editColumn('order_status', function(Order $data) {
         ...
         return '<div class="btn-group">
                     ...
                     <div class="dropdown-menu">
                         <a class="dropdown-item StatusCheck" data-val="Pending" href="javascript:;" data-href="'.route('user-tour-order-status',[ $data->id, 'Pending']).'">'.__('Pending').'</a>
                         <a class="dropdown-item StatusCheck" data-val="Processing" href="javascript:;" data-href="'.route('user-tour-order-status',[ $data->id, 'Processing']).'">'.__('Processing').'</a>
                         ...
                         <a class="dropdown-item StatusCheck" data-val="Completed" href="javascript:;" data-href="'.route('user-tour-order-status',[$data->id, 'Cancel']).'">'.__('Cancel').'</a>
                     </div>
                  </div>';
         })
    ...
    {
      $data = Order::findOrFail($id);
      return view('user.tour.orders.details',compact('data'));
    }
