...
class SpaceOrderController extends Controller
{
   ...
   public function __construct()
   {
    ...
    {
      if(!Helper::tourCheck() == true){
         return redirect(route('user-space-index'));
      }
        return view('user.space.orders.index');
    }
    ...
    public function datatables($type)
    {
        $user = Auth::user();
        if($type == 'all'){
         $datas = Order::orderBy('id','desc')->where('order_type','Space')->where('author_type','user')->where('author_id',$user->id)->get();
        }else{
         $datas = Order::orderBy('id','desc')->where('order_status',$type)->where('order_type','Space')->where('author_type','user')->where('author_id',$user->id)->get();
        }
        ...
        ->editColumn('order_status', function(Order $data) {
         ...
         return '<div class="btn-group">
                     ...
                     <div class="dropdown-menu">
                         <a class="dropdown-item StatusCheck" data-val="Pending" href="javascript:;" data-href="'.route('user-space-order-status',[ $data->id, 'Pending']).'">'.__('Pending').'</a>
                         <a class="dropdown-item StatusCheck" data-val="Processing" href="javascript:;" data-href="'.route('user-space-order-status',[ $data->id, 'Processing']).'">'.__('Processing').'</a>
                         ...
                         <a class="dropdown-item StatusCheck" data-val="Cancel" href="javascript:;" data-href="'.route('user-space-order-status',[$data->id, 'Cancel']).'">'.__('Cancel').'</a>
                         </div>
                  </div>';
         })
       ...
       ->addColumn('action', function(Order $data) {
           ...
           <a href="javascript:;" data-href="' .$data->order_status != 'Cancel' ? route('admin-space-order-delete',$data->id) : 'javascript:;' . '" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i></a>
       </div>';
       })
    ...
    public function SpaceordersDetails($id)
    {
      ...
      $data = Order::findOrFail($id);
      return view('user.space.orders.details',compact('data'));
    }
