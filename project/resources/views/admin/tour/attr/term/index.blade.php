...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('Attribute : ') }}{{$attribute->name}}  <a href="{{ route('admin-tourattr-index') }}" class="btn back-btn btn-sm">{{__('Back')}}</a></h1>
        <ol class="breadcrumb">
    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            <div class="card mb-4">
                ...
                <!-- Form Basic -->
                <div class="card-body">
                    <form id="pageFormTable" action="{{route('admin-tourterm-store')}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label for="name">{{ __('Name') }}</label>
                            <input type="text" class="form-control" name="name" value="" id="name" placeholder="{{ __('Name') }}">
                        </div>
                        <input type="hidden" value="{{$attribute->id}}" name="tour_attr_id">
            ...
            <div class="card mb-4">
                <div class="card-header">
                    <form action="{{route('admin.bulk.delete')}}" id="bulk-delete" class="d-flex flex-row align-items-center">
                        <input type="hidden" name="table" value="tour_terms">
     ...
     $(function ($) {
       "use strict";
		...
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('admin-tourtrem-datatables',$attribute->id) }}',
               columns: [
