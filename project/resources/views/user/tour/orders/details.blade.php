...
<ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
  ...
  <div class="tab-content" id="pills-tabContent">
    <div class="tab-pane fade show active" id="pills-tone" role="tabpanel" aria-labelledby="pills-tone-tab">
      ...
      @endif
      @if($data->person_price && $data->person_type)
        <ul class="list-group">
            <li class="list-group-item d-flex justify-content-between">
                <b>{{__('Person Prices')}}:</b>
                <span></span>
            </li>
            @foreach (explode(',',$data->person_type) as  $key => $p_type)
                <li class="list-group-item d-flex justify-content-between">
                    <span>{{$p_type}} :
                    </span>
                    <span>{{$data->currency_sign}} {{round(explode(',',$data->person_price)[$key] * $data->currency_value ,2)}}
                    </span>
                </li>
            @endforeach
        </ul>
