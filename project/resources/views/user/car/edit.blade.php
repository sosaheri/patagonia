...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('Edit Car') }}
    <a href="{{ url()->previous() }}" class="btn back-btn btn-sm">{{__('Back')}}</a>
    </h1>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{route('user-dashboard')}}">{{ __('Dashboard') }}</a></li>
            <li class="breadcrumb-item"><a href="{{route('user-car-index')}}">{{ __('Car') }}</a></li>
            <li class="breadcrumb-item active" aria-current="page">{{ __('Edit Car') }}</li>
        </ol>
    </div>
    @include('includes.admin.form-error') @include('includes.admin.form-success')
    <form action="{{route('user-car-update',$main->id)}}" enctype="multipart/form-data" method="POST" id="pageForm">
        @csrf
        <div class="row">
            <div class="col-lg-8">
                <!-- Form Basic -->
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="title" id="title" placeholder="{{ __('Title') }}" value="{{$main->title}}">
                        </div>
                        ...
                        <div id="faq-section">
                            @if($main->faq_title && $main->faq_content)
                             @foreach (explode(',,,',$main->faq_title) as $faq_key => $faq_title)
                            @php
                              $content = explode(',,,',$main->faq_content);
                            @endphp
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="text-center banner_image_view mb-2">
                                        <img class="img-profile" src="{{$main->banner_image ? asset('assets/images/car/banner-image/'.$main->banner_image) : asset('assets/images/noimage.png')}}" >
                                    </div>
                        ...
                        <div class="row border-buttom">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="passenger" id="passenger" placeholder="{{ __('Passenger') }}" value="{{$main->passenger}}">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="gear">{{ __('Gear Shift') }}</label>
                                    <input type="text" class="form-control" name="gear" id="gear" placeholder="{{ __('Gear Shift') }}" value="{{$main->passenger}}">
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="baggage" id="baggage" placeholder="{{ __('Baggage') }}" value="{{$main->baggage}}">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="door">{{ __('Door') }}</label>
                                    <input type="number" class="form-control" name="door" id="door" placeholder="{{ __('Door') }}" value="{{$main->door}}">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card mb-4 p-3">
                    ...
                    <div class="card-body">
                        <div class="row border-buttom">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="number" id="number" placeholder="{{ __('Number') }}" value="{{$main->number}}">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="sale_price" id="sale_price" placeholder="{{ __('Car Previous Price') }}" value="{{PriceHelper::showAdminPrice($main->sale_price)}}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="custom-control custom-switch my-3">
                            ...
                            <label class="custom-control-label" for="extra-price">{{__('Enable extra price')}}</label>
                        </div>

                        <div class="show-extra-price {{$main->extra_price ? '' : 'd-none'}}">
                            <div id="price-section">
                                @if($main->extra_price_name && $main->extra_price_type)
                                        ...
                                        <div class="col-sm-4 col-md-4 col-4">
                                            <div class="form-group ">
                                                <select name="extra_price_type[]" class="form-control" required>
                                                    <option value="dey" {{$type=='dey' ? 'selected' : ''}}>Per Day</option>
                                                    <option value="month" {{$type=='month' ? 'selected' : ''}}>Per Month</option>
                                                    <option value="year" {{$type=='year' ? 'selected' : ''}}>Per Year</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
            ...
            <div class="col-md-4">
                ...
                <div class="card mt-3">
                    ...
                    <div class="card-body">
                        ...
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input term-check" rel="{{$term->car_attr_id}}" name="term_id[]" value="{{$term->id}},{{$term->car_attr_id}}" id="{{$term->id}}" {{in_array($term->id,explode(',',$main->attr_term_id)) ? 'checked' : ''}}>
                ...
                <div class="card mt-3">
                    ...
                    <div class="card-body">
                        <div class="text-center image-view mb-4">
                            <img class="img-profile" src="{{$main->image->image ? asset('assets/images/car/feature-image/'.$main->image->image) : asset('assets/images/noimage.png')}}" >
                        </div>
                        <div class="custom-file">
			...
			<div class="modal-body">
				...
				<div class="gallery-images">
					<div class="selected-image">
						<div class="row">
                            @foreach ($main->gallery->where('type','car') as $key => $gallery)
                            <div class="col-sm-6">
                                <div class="img gallery-img"><span class="remove-img" data-href="{{route('car.gallery.image.remove',$gallery->id)}}"><i class="fas fa-times"></i><input type="hidden" value="{{$key}}"></span>
                                    <a href="{{asset('assets/images/car/gallery/'.$gallery->image)}}" target="_blank"><img src="{{asset('assets/images/car/gallery/'.$gallery->image)}}" alt="gallery image"></a>
                                </div>
                            </div>
                            @endforeach
						</div>
